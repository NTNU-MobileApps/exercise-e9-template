...
class CartItem {
  ...
  /// Create a CartItem from a map of values (JSON). The map is expected to
  /// have the necessary attributes: name, size, count
  /// If any of the attributes is missing, null is returned
  static CartItem? fromMap(Map<String, dynamic> data) {
    return CartItem(data["name"], data["size"], data["count"]);
  }
}
